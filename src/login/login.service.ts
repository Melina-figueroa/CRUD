...
export class LoginService {
  ...
  public async getAll(): Promise<Login[]> {
    try {
      ...
      if (this.usuarios) return this.usuarios;
      else throw new Error('No se encuentran usuarios.');
    } catch (error) {
  ...
  public async getById(id: string): Promise<Login[]> {
    try {
      ...
      this.usuarios = [];
      if (login) this.usuarios.push(login);
      ...
      throw new HttpException(
        {
          status: HttpStatus.NOT_FOUND,
          error: 'Error en la busqueda de login ' + id + ' : ' + error,
        },
        HttpStatus.NOT_FOUND,
      );
    }
    ...
    try {
      if (datos) {
        if (datos.usuario && datos.palabra) {
          ...
          let login: Login = await this.loginRepository.findOne(criterio);
          if (login) {
            if (login.getPalabra() == datos.palabra) return 'ok';
            else return 'La contraseña no es valida';
          } else {
            await this.loginRepository.save(
              new Login(datos.usuario, datos.palabra),
            );
          }
