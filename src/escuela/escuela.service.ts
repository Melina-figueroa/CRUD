...
export class EscuelaService {
  ...
  public async getAllRaw(): Promise<Escuela[]> {
    try {
      let escuela = await this.escuelaRepository.query(
  ...
  public async getById(id: number): Promise<Escuela[]> {
    try {
      const criterio: FindOneOptions = {
        relations: ['ciudad', 'clases'],
        where: { idEscuela: id },
      };
      let escuela: Escuela = await this.escuelaRepository.findOne(criterio);
      this.escuelas = [];
      if (escuela) this.escuelas.push(escuela);
      else throw new Error('La escuela no se encuentra.');
      return this.escuelas;
    } catch (error) {
      throw new HttpException(
        {
      ...
      if (datos)
        if (datos.idEscuela && datos.nombre && datos.domicilio && datos.ciudad)
          if (await this.existeEscuela(datos.idEscuela)) {
            ...
            await this.escuelaRepository.save(
              new Escuela(
                datos.idEscuela,
                datos.nombre,
                datos.domicilio,
        ...
        else throw new Error('Los datos para crear escuela no son validos');
      else throw new Error('No hay datos para crear escuela');
      return 'ok';
    } catch (error) {
  ...
  public async delete(id: number): Promise<string> {
    try {
      ...
      else throw new Error('No hay datos para eliminar escuelas');
      return 'ok';
    } catch (error) {
      return error.message;
    }
      ...
      if (datos)
        if (datos.idEscuela && datos.nombre && datos.domicilio && datos.ciudad)
          if (await this.existeEscuela(id)) {
            ...
            let escuela: Escuela =
              await this.escuelaRepository.findOne(criterio);
        ...
        else throw new Error('Los datos para modificar escuela no son validos');
      else throw new Error('No hay datos para modificar escuelas');
      return 'ok';
    } catch (error) {
      return error.message;
    }
