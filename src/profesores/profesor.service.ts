...
export class ProfesorService {
  ...
  public async getAllRaw(): Promise<Profesor[]> {
    try {
      let profesor = await this.profesorRepository.query(
        'SELECT * FROM profesores',
      );
      profesor.forEach((element) => {
        let profesor: Profesor = new Profesor(
          element['idProfesor'],
          element['apellidoNombres'],
        );
  ...
  public async getById(id: number): Promise<Profesor[]> {
    try {
      ...
      this.profesores = [];
      if (profesor) this.profesores.push(profesor);
      else throw new Error('La profesor no se encuentra.');
      return this.profesores;
    } catch (error) {
      throw new HttpException(
        {
          status: HttpStatus.NOT_FOUND,
          error: 'Error en la busqueda de profesor ' + id + ' : ' + error,
        },
        HttpStatus.NOT_FOUND,
      );
    }
      ...
      if (datos)
        if (datos.idProfesor && datos.apellidoNombres)
          if (await this.existeProfesor(datos.idProfesor)) {
            ...
            await this.profesorRepository.save(
              new Profesor(datos.idProfesor, datos.apellidoNombres),
            );
          }
        else throw new Error('Los datos para crear profesor no son validos');
      else throw new Error('No hay datos para crear profesor');
      return 'ok';
    } catch (error) {
  ...
  public async delete(id: number): Promise<string> {
    try {
      if (id)
        ...
        } else throw new Error('El profesor no se encuentra.');
      else throw new Error('No hay datos para eliminar profesores');
      return 'ok';
    } catch (error) {
      return error.message;
    }
  ...
  public async update(id: number, datos: any): Promise<string> {
    try {
        ...
        if (datos.idProfesor && datos.apellidoNombres)
          if (await this.existeProfesor(id)) {
            ...
            let profesor: Profesor =
              await this.profesorRepository.findOne(criterio);
      ...
      else throw new Error('No hay datos para modificar profesores');
      return 'ok';
    } catch (error) {
      return error.message;
    }
